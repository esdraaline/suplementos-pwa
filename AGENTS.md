...
Este projeto é acompanhado pelo Mentor de Projetos. Slug: `suplementos-pwa`. O estado executivo vive em `.project-mentor/project.yaml` e o histórico em `.project-mentor/sessions/`. **Nunca edite esses arquivos à mão**: toda escrita passa pelo CLI `mentor`.
...
2. Rode `mentor project suplementos-pwa --brief` e leia a última sessão em `.project-mentor/sessions/`.
...
4. Pergunte o objetivo só se não estiver claro. Depois rode `mentor start suplementos-pwa --objective "..." --agent <claude-code|codex|antigravity|copilot>`.
...
- Ação nova: `mentor action add suplementos-pwa --title "..."`. Concluir: `mentor action done suplementos-pwa <act-id> --level implemented|tested`. Bloqueio: `mentor blocker add suplementos-pwa --description "..."`.
...
2. Rode `mentor close suplementos-pwa --from <rascunho.md>` e mostre o resultado da validação. Se falhar, mostre o erro e **não** finja sucesso.
